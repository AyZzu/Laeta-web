...
const VideoMila = ({ data }) => {
  useEffect(() => {
    AOS.init();
    AOS.refresh();
  }, []);

  const VideoMilas = data.map((items, i) => {
    return (
      <div className="listVideo" key={i}>
        <div>
          <a href={`https://www.youtube.com/watch?v=${items.snippet.resourceId.videoId}`}>
            <img src={items.snippet.thumbnails.maxres.url} width={290} data-aos="fade-up" data-aos-duration="900" />
          </a>
        </div>
  ...
  return (
    <div className="listVideoMila">
      <div className="wrapperListVideo">
        ...
        <div className="videoList">{VideoMilas}</div>
      </div>
    </div>
  );
};

export default VideoMila;
