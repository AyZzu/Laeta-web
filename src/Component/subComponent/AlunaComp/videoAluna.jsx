...
import "../AlunaComp/videoAluna.css";
...
const VideoAluna = ({ data }) => {
  useEffect(() => {
    AOS.init();
    AOS.refresh();
  }, []);

  const VideoAluna = data.map((items, i) => {
    return (
  ...
  return (
    <div className="listVideoAluna">
      <div className="wrapperListVideo">
        ...
        <div className="videoList">{VideoAluna}</div>
      </div>
    </div>
  );
};

export default VideoAluna;
