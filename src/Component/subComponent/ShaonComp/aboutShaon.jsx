...
import { getDebutShaon } from "../../YtApi";

import "../ShaonComp/aboutShaon.css";
...
const AboutShaon = () => {
  const [debutShaons, setDebutShaons] = useState([]);

  useEffect(() => {
    AOS.init();
    AOS.refresh();

    getDebutShaon().then((items) => {
      setDebutShaons(items);
    });
  }, []);

  const DebutShaon = debutShaons.map((items, i) => {
    return (
      <div className="videoDebutShaons" key={i}>
        ...
        <div className="debutThumb">
          ...
          <div className="textDebut" data-aos="fade-up" data-aos-duration="700">
            ...
            <p>Hi! I’m Shaon River B. Bassist turned solo singer... and maybe your next favorite voice~</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="aboutShaon">
      ...
      <div className="Lore" data-aos="fade-up" data-aos-duration="650">
        <p>Seorang bassist yang hampir mati tetapi diselamatkan oleh seseorang dan memilih untuk menjadi vtuber.</p>
      </div>

      {/* DEBUT VIDEO */}
      <div className="debutVideo">{DebutShaon}</div>

      {/* FRIEND */}
      <div className="Hatline" data-aos="fade-up" data-aos-duration="700">
        <h1>Friend's</h1>
      </div>
      <div className="friendsShaon">
