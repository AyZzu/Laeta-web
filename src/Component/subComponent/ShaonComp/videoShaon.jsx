...
import "../ShaonComp/videoShaon.css";
...
const VideoShaon = ({ data }) => {
  useEffect(() => {
    AOS.init();
    AOS.refresh();
  }, []);

  const VideoShaon = data.map((items, i) => {
    return (
  ...
  return (
    <div className="listVideoShaon">
      <div className="wrapperListVideo">
        ...
        <div className="videoList">{VideoShaon}</div>
      </div>
    </div>
  );
};

export default VideoShaon;
