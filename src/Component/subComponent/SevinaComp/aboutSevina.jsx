...
import { getDebutSevina } from "../../YtApi";

import "../SevinaComp/aboutSevina.css";
...
const AboutSevina = () => {
  const [debutSevinas, setDebutSevinas] = useState([]);

  useEffect(() => {
    AOS.init();
    AOS.refresh();

    getDebutSevina().then((items) => {
      setDebutSevinas(items);
    });
  }, []);

  const DebutSevina = debutSevinas.map((items, i) => {
    return (
      <div className="videoDebutMila" key={i}>
        ...
        <div className="debutThumb">
          ...
          <div className="textDebut" data-aos="fade-up" data-aos-duration="700">
            ...
            <p>Hi Honey.. Your Virtual Alchemist Tiger here. Would you be my guinea pig? 🐯🧪👩🏻‍🔬</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="aboutSevina" >
      ...
      {/* DEBUT VIDEO */}
      <div className="debutVideo">{DebutSevina}</div>
